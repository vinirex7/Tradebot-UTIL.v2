...
class PriceFeed:
    ...
    def _download_mt5(
        ...
        """
        Baixa dados históricos via MT5.
        Sempre usa a sessão já aberta (initialize sem credenciais).
        """
        if not _MT5_AVAILABLE:
            return None
        try:
            import MetaTrader5 as mt5  # noqa
            # Conecta à sessão existente sem credenciais
    ...
    def _mt5_last_price(self, ticker: str) -> Optional[float]:
        """
        Obtém preço atual via MT5, sempre usando sessão já aberta.
        """
        if not _MT5_AVAILABLE:
            return None
        try:
            import MetaTrader5 as mt5  # noqa
            # Garante conexão ativa sem re-autenticar
            if not mt5.initialize():
                return None
